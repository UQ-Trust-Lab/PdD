...
class MixGenerator:
    def __init__(
        ...
        distribution: str,
        density: float,
        diversity_dict: Dict[str, List[str]] = None,
        ...
        **kwargs
        ...
        :param density: The density of perturbations, e.g., 0.1 means 10% of the
            indexes are perturbed.
        :param diversity_dict: The diversity dictionary, which describes the candidate
            perturbed characters for each index.
        :param insert_mode: The insert mode, which is used to insert perturbations into
            the original string.
        :param kwargs: The keyword arguments, which are used to set the distribution of
            perturbations, e.g., mean and std.
        """
        assert 0.0 <= density <= 1.0, "The density should be in the range of [0, 1]."
        assert distribution in [
            "uniform",
            "normal",
        ], 'The distribution should be "uniform" or "normal".'
    ...
    def generate(self, string_ori: str) -> str:
        ...
        for idx in idxes_ptb:
            ...
            random_idx = np.random.randint(
                0, len(self.diversity_dict[char_ptb])
            )  # Choose a random perturbed character
            while (
                self.diversity_dict[char_ptb][random_idx] == char_ptb
            ):  # Avoid choosing the same character
            ...
            string_ptb[idx] += self.diversity_dict[char_ptb][
                random_idx
            ]  # Perturb the character
